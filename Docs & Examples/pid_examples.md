...
inline PID liftPID{0.45, 0, 0, 0, "Lift"};
```



## User Control
This is the simplest implementation of our PID library PID.  
...
The exit conditions used in the PID class are the same as all drive movements for our PID library  You can see how exit conditions work [here](/tutorials/exiting_movements).  
...
  exit_output exit = RUNNING;
  while (liftPID.exit_condition({l_lift, r_lift}, true) == RUNNING) {
    set_lift(liftPID.compute(l_lift.get_position()));
    pros::delay(util::DELAY_TIME);
  }
  set_lift(0);
...
```cpp
void lift_task() {
  pros::delay(2000);  // Set our PID library calibrate before this function starts running
  while (true) {
    set_lift(liftPID.compute(l_lift.get_position()));
